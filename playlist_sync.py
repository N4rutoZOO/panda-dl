...
def spotify_playlists(sid: str):
    ...
    return [
        {"id": p.get("id"), "name": p.get("name"), "count": (p.get("items") or {}).get("total", 0), "image": ((p.get("images") or [{}])[0]).get("url")}
        for p in items if p.get("id")
    ]


def spotify_tracks(sid: str, playlist_id: str, limit: int):
    raw = _spotify_pages(sid, f"https://api.spotify.com/v1/playlists/{playlist_id}/items", {"limit": 50, "additional_types": "track"})
    tracks = []
...
def spotify_search(sid: str, track: dict[str, Any]):
    ...
    for item in ((data.get("tracks") or {}).get("items") or []):
        artists = ", ".join(a.get("name", "") for a in item.get("artists") or [])
        out.append({"id": item.get("id"), "title": item.get("name") or "", "artist": artists, "isrc": (item.get("external_ids") or {}).get("isrc"), "duration_ms": item.get("duration_ms")})
    return out


def spotify_create_playlist(sid: str, name: str):
    data = _api("spotify", sid, "POST", "https://api.spotify.com/v1/me/playlists", json={"name": name[:100], "public": False, "description": "Transférée avec Dingo-dl"})
    return data.get("id")


def spotify_add_tracks(sid: str, playlist_id: str, ids: list[str]):
    for start in range(0, len(ids), 100):
        _api("spotify", sid, "POST", f"https://api.spotify.com/v1/playlists/{playlist_id}/items", json={"uris": [f"spotify:track:{x}" for x in ids[start:start + 100]]})
...
def youtube_playlists(sid: str):
    ...
    return [{"id": p.get("id"), "name": (p.get("snippet") or {}).get("title"), "count": (p.get("contentDetails") or {}).get("itemCount", 0), "image": ((((p.get("snippet") or {}).get("thumbnails") or {}).get("medium") or {}).get("url"))} for p in items if p.get("id")]


def youtube_tracks(sid: str, playlist_id: str, limit: int):
    raw = _youtube_pages(sid, "playlistItems", {"part": "snippet,contentDetails", "playlistId": playlist_id, "maxResults": 50})[:limit]
    ids = [(x.get("contentDetails") or {}).get("videoId") or (((x.get("snippet") or {}).get("resourceId") or {}).get("videoId")) for x in raw]
    ids = [x for x in ids if x]
    durations: dict[str, int] = {}
    for start in range(0, len(ids), 50):
        videos = _api("youtube", sid, "GET", "https://www.googleapis.com/youtube/v3/videos", params={"part": "contentDetails", "id": ",".join(ids[start:start + 50])})
    ...
    for item in raw:
        ...
        tracks.append(UnifiedTrack(id=video_id, title=title, artist=artist or snippet.get("videoOwnerChannelTitle") or "", duration_ms=durations.get(video_id), url=f"https://www.youtube.com/watch?v={video_id}", source="youtube").model_dump())
    return tracks


def youtube_search(sid: str, track: dict[str, Any]):
    q = f'{track.get("artist") or ""} {track["title"]}'.strip()
    data = _api("youtube", sid, "GET", "https://www.googleapis.com/youtube/v3/search", params={"part": "snippet", "type": "video", "maxResults": 5, "q": q})
    items = data.get("items") or []
    ids = [((x.get("id") or {}).get("videoId")) for x in items]
    ids = [x for x in ids if x]
    durations = {}
    if ids:
        videos = _api("youtube", sid, "GET", "https://www.googleapis.com/youtube/v3/videos", params={"part": "contentDetails", "id": ",".join(ids)})
        for v in videos.get("items") or []:
            durations[v.get("id")] = _iso_duration_ms(((v.get("contentDetails") or {}).get("duration") or ""))
    out = []
    for item in items:
        ...
        snippet = item.get("snippet") or {}
        title, artist = _split_youtube_title(snippet.get("title") or "")
        out.append({"id": vid, "title": title, "artist": artist or snippet.get("channelTitle") or "", "duration_ms": durations.get(vid), "isrc": None})
    return out


def youtube_create_playlist(sid: str, name: str):
    data = _api("youtube", sid, "POST", "https://www.googleapis.com/youtube/v3/playlists", params={"part": "snippet,status"}, json={"snippet": {"title": name[:150], "description": "Transférée avec Dingo-dl"}, "status": {"privacyStatus": "private"}})
    return data.get("id")


def youtube_add_tracks(sid: str, playlist_id: str, ids: list[str]):
    for video_id in ids:
        _api("youtube", sid, "POST", "https://www.googleapis.com/youtube/v3/playlistItems", params={"part": "snippet"}, json={"snippet": {"playlistId": playlist_id, "resourceId": {"kind": "youtube#video", "videoId": video_id}}})
...
def _provider_add(provider: str, sid: str, playlist_id: str, ids: list[str]):
    spotify_add_tracks(sid, playlist_id, ids) if provider == "spotify" else youtube_add_tracks(sid, playlist_id, ids)
...
def _run_transfer(job_id: str, sid: str, req: dict[str, Any]):
    try:
        ...
        for idx, track in enumerate(tracks, 1):
            ...
            report.append({"index": idx - 1, "source": track, "matched": matched, "score": score, "destination": best if matched else None})
            _update_job(job_id, progress=5 + int((idx / total) * 80), stage="matching", current=idx, total=total, matched=len(matched_ids), unmatched=idx - len(matched_ids))
        _update_job(job_id, stage="writing", progress=88)
        if matched_ids:
            _provider_add(dest, sid, destination_playlist_id, matched_ids)
        _update_job(job_id, status="ready", stage="ready", progress=100, matched=len(matched_ids), unmatched=total - len(matched_ids), total=total, destination_playlist_id=destination_playlist_id, report=report)
...
def providers(request: Request):
    ...
    return {"providers": [
        {"id": "spotify", "name": "Spotify", "configured": bool(SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET), "connected": "spotify" in connected},
        {"id": "youtube", "name": "YouTube / YouTube Music", "configured": bool(GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET), "connected": "youtube" in connected},
    ]}
...
def oauth_start(provider: str, request: Request):
    ...
    if provider == "spotify":
        ...
        params = {"client_id": SPOTIFY_CLIENT_ID, "response_type": "code", "redirect_uri": redirect, "state": state, "scope": "playlist-read-private playlist-read-collaborative playlist-modify-private playlist-modify-public", "show_dialog": "true"}
    ...
    if provider == "youtube":
        ...
        params = {"client_id": GOOGLE_CLIENT_ID, "redirect_uri": redirect, "response_type": "code", "scope": "https://www.googleapis.com/auth/youtube", "access_type": "offline", "prompt": "consent", "state": state}
...
def oauth_callback(provider: str, request: Request, code: str, state: str):
    ...
    if provider == "spotify":
        r = requests.post("https://accounts.spotify.com/api/token", data={"grant_type": "authorization_code", "code": code, "redirect_uri": redirect}, auth=(SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET), timeout=HTTP_TIMEOUT)
    elif provider == "youtube":
        r = requests.post("https://oauth2.googleapis.com/token", data={"grant_type": "authorization_code", "code": code, "redirect_uri": redirect, "client_id": GOOGLE_CLIENT_ID, "client_secret": GOOGLE_CLIENT_SECRET}, timeout=HTTP_TIMEOUT)
    else:
        raise HTTPException(status_code=404, detail="Plateforme inconnue")
    if not r.ok:
...
def transfer(body: TransferRequest, request: Request):
    ...
    with SYNC_LOCK:
        SYNC_JOBS[job_id] = {"id": job_id, "status": "queued", "stage": "queued", "progress": 0, "created_at": now, "updated_at": now, "matched": 0, "unmatched": 0}
